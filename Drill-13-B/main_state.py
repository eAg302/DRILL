...
def collide_left(a, b):
    # fill here
    left_a, bottom_a, right_a, top_a = a.get_bb()
    left_b, bottom_b, right_b, top_b = b.get_left_bb()
...
def update():
    ...
    for brick in bricks.copy():
        if collide_left(boy, brick):
            print('brick left collide')
            boy.cur_state = RunState
            if collide_top(boy,brick):
                boy.x = clamp(brick.x-90, boy.x, brick.x+90)
                boy.y = brick.y+50


            #boy.velocity = brick.speed + boy.velocity
